...
{
	SourceHook::CVector<SpawnData> m_SpawnData;
	...
	bool Teleport(ZIPlayer *player)
	{
		...
		if( !player->m_IsInfected && protectionDuration > 0.0f )
		{
			...
			player->m_pRemoveProtectionTimer = timersys->CreateTimer(&ZITimersCallback::m_RemoveProtection, protectionDuration, player, TIMER_FLAG_NO_MAPCHANGE);
		}
